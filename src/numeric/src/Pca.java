...
public class Pca {
     ...
     * <p>
     * it is just the simple gradient descend on the constraint manifold
     *
     * @param conv
     * @return
     */
...
//			System.out.println(eigen[i]);
    ...
    public Vector[] getNPca(Vector[] Data, int n) {
        ...
        average = myu;
        /**
         * subtract the average to the data, and compute covariance matrix
         */
        ...
        symmetricEigen.computeEigen(1E-10, n);
        eigenVectors = symmetricEigen.getEigenVectors();
        Double[] eigenValues = symmetricEigen.getEigenValues();
        for (int i = 0; i < n; i++) {
            this.eigenValues[i] = Math.sqrt(Math.abs(eigenValues[i]));
        }

        for (int i = 0; i < n; i++) {
            ans[i] = eigenVectors[i].copy();
        }
        return ans;
    }
