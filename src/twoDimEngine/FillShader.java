...
public class FillShader extends Wireframe {
	...
	private void paintPoly(Element2D element2d) {
		...
		for (int i = 0; i < length; i++) {
			x[i] = engine.IntegerCoordX(vec[i].getX());
			y[i] = engine.IntegerCoordY(vec[i].getY());
		}
	...
	public void paintPoint(Point2D element) {
		Graphics g = engine.getImageGraphics();
		int i = engine.IntegerCoordX(element.getVertex(0).getX());
		int j = engine.IntegerCoordY(element.getVertex(0).getY());
