...
public class SparseHeatFlow implements HeatFlow {
	...
	public void heatFlow(double lambda, LowBow l) {
		...
		do {
			...
			for (int i = 1; i < l.curve.length - 1; i++) {
				 ...
				 * second derivative
				 */
				Vector d2x = Vector.add(Vector.diff(l.heatCurve[i + 1], Vector.scalarProd(2, l.heatCurve[i])), l.heatCurve[i - 1]);
				d2x = Vector.scalarProd(lambda * (l.samples - 1), d2x);
				/**
				 * distance from the original l.curve
				 */
				grad[i] = Vector.scalarProd((1 - lambda) * l.step, Vector.diff(l.curve[i], l.heatCurve[i]));
				/**
			...
			for (int j = 0; j < 3; j++) {
				...
				for (int i = 1; i < l.curve.length - 1; i += 2) {
					 ...
					 * update l.curve
					 */
					auxCurve[i] = Vector.add(l.heatCurve[i], Vector.scalarProd(delta, grad[i]));
					auxCurve[i + 1] = Vector.add(l.heatCurve[i + 1], Vector.scalarProd(delta, grad[i + 1]));
					/**
			 ...
			 */
			for (int i = 1; i < l.curve.length - 1; i += 2) {
				l.heatCurve[i] = Vector.add(l.heatCurve[i], Vector.scalarProd(dt, grad[i]));
				l.heatCurve[i + 1] = Vector.add(l.heatCurve[i + 1], Vector.scalarProd(dt, grad[i + 1]));
			}
