...
function createVideoTag(uID, clusterId, videoAddressUrl, videoId) {
    var address = "ArcSummary/summary" + uID + "/" + clusterId + "/" + videoAddressUrl;
...
function checkIfInputAreFilled() {
    var listOfRequiredInput = [];
    listOfRequiredInput.push($("#baseFolder"));
    for(var i = 0; i < listOfRequiredInput.length; i++) {
        if(listOfRequiredInput[i].val() === "") {
            listOfRequiredInput[i].attr("placeholder", "Value is missing!!**")
     ...
     $.ajax({
          ...
          success: function( result ) {
            ...
            for(var i = 0; i < split.length-1; i++ ) {
                $("#clusteringMethod").append("<option>" + split[i] + "</option>");
            }
          }
        });
}
...
$("#videoExtension").change(function() {
        if($("#videoExtension").val() === "other") {
            $("#videoExtensionOther").slideDown();
        } else {
            $("#videoExtensionOther").slideUp();
        }
});
$("#stopWordsMethod").change(function() {
        if($("#stopWordsMethod").val() === "Entropy") {
            $("#entropyMethod").slideDown();
            $("#entropy").val(0.05);
        } else {
            $("#entropyMethod").slideUp();
            $("#entropy").val(0.0);
        }
})
...
$("#knnDefaultButton").click(function() {
        if($("#knnDefaultButton").text() === "Default") {
            $("#knn").val(0);
            $("#knn").slideUp();
            $("#knnDefaultButton").text("Custom")
        } else {
            $("#knn").val(1);
            $("#knn").slideDown();
            $("#knnDefaultButton").text("Default")
        }
